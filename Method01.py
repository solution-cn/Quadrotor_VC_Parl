...
reward_max = -1027
while True:
    step, reward = run_episode(env, agent, rpm, type_A)
    total_step += step
    ...
    if total_step // Test_Round == test_flag:
        ...
        if reward_max < Test_Reward[5]:
            ...
            model_path = './R_%.0f_Step%d.ckpt' % (reward_max, total_step)
            agent.save(model_path)
        if total_step >= Train_Step:
            model_path = './R_%.0f_Over.ckpt' % (Test_Reward[5])
            agent.save(model_path)
            break
        test_flag += 1
